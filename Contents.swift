...
var num_humans = humanOfficers.count



// Q1: What percentage of humans are female?
var human_females = femaleOfficers.filter {humanOfficers.contains($0)}
var num_female_humans = human_females.count
var percentage_female = Float(num_female_humans)/Float(num_humans)




// Q2: How many first officers are human?
var first_officers_human = firstOfficers.filter {humanOfficers.contains($0.0)}
var num_first_officers_human = first_officers_human.count



// Q3: Get a list of females sorted by name
var femaleOfficersSorted = femaleOfficers.sorted(by: <)
...
let all_captains = captains.map{ Officer(name: $0, ship: $1) }



// Q5: Alphabetize the previous list by the captain's name
var all_captains_alpha = captains.map{ Officer(name: $0, ship: $1) }.sorted(by: {$0 < $1})
...
var all_first_officers = firstOfficers.map{ Officer(name: $0, ship: $1) }
var vulcan_first_officers = all_first_officers.filter {vulcanOfficers.contains($0.name)}
print(vulcan_first_officers.count)
print(vulcan_first_officers[0].name)
print(vulcan_first_officers[1].name)




// Q7: How many these Vulcan first officers have names shorter than 5 characters?
var vulcan_first_officers_short_name = vulcan_first_officers.filter {$0.name.count < 5}
print(vulcan_first_officers_short_name.count)
print(vulcan_first_officers_short_name[0].name)
